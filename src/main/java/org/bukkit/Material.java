...
public enum Material implements Keyed, Translatable, net.kyori.adventure.translation.Translatable { // Paper
    ...
    /**
     * Add a new material from Forge mods.
     * 
     * STACKMANIA FIX: Uses MaterialCacheManager to prevent double-injection issues.
     * When a mod adds both a block and item with the same name, this method is called twice.
     * The cache ensures only one Material instance exists, with proper block/item flags.
     * 
     * @param materialName The normalized material name
     * @param id The item/block ID
     * @param stack Max stack size
     * @param isBlock True if this is a block
     * @param isItem True if this is an item
     * @param resourceLocation The Forge ResourceLocation
     * @return The Material instance (existing or newly created)
     */
    public static Material addMaterial(String materialName, int id, int stack, boolean isBlock, boolean isItem, ResourceLocation resourceLocation) {
        // STACKMANIA: Check cache first to prevent duplicates
        Material existing = BY_NAME.get(materialName);
        
        if (existing != null) {
            // Material already exists - update flags instead of creating duplicate
            ...
            // Ensure key is set
            if (existing.key == null && resourceLocation != null) {
                existing.key = CraftNamespacedKey.fromMinecraft(resourceLocation);
            }
            return existing;
        }
        ...
        Material material = MohistDynamEnum.addEnum(
            Material.class, 
            materialName, 
            List.of(Integer.TYPE, Integer.TYPE, Boolean.TYPE, Boolean.TYPE), 
            List.of(id, stack, isBlock, isItem)
        );
        
        if (material != null) {
            // Set additional flags
            material.isForgeBlock = isBlock;
            material.isForgeItem = isItem;
            
            // Register in lookup map
            BY_NAME.put(materialName, material);
            
            // Set the namespaced key
